(() => {
  ...
  function drawLantern(ps, p) {
    ...
    // Tiered attachments: 10 brass · 20 bound · 30 crown · 40 sovereign
    if (p.level >= 10) {
      const cy = ps.y + bob - 10;
      const tier = p.level >= 40 ? 4 : p.level >= 30 ? 3 : p.level >= 20 ? 2 : 1;
      const gemN = tier >= 4 ? 7 : tier >= 2 ? 5 : 3;
      const gemR = tier >= 4 ? 48 : tier >= 2 ? 44 : 40;
      ctx.save();
      // mid band
      ...
      if (tier >= 2) {
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.ellipse(ps.x, cy - 8, 48, 16, -0.2, 0, TAU);
        ctx.stroke();
        // side blades
        ctx.fillStyle = "rgba(184, 115, 51, 0.85)";
        for (const side of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(ps.x + side * 36, cy + 8);
          ctx.lineTo(ps.x + side * 58, cy - 6);
          ctx.lineTo(ps.x + side * 40, cy + 22);
          ctx.closePath();
          ctx.fill();
        }
      }
      if (tier >= 3) {
        ctx.strokeStyle = "rgba(255, 192, 80, 0.9)";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.ellipse(ps.x, cy - 52, 26, 8, -0.1, 0, TAU);
        ctx.stroke();
        ctx.fillStyle = "rgba(240, 180, 41, 0.9)";
        for (let i = 0; i < 5; i++) {
          const a = -Math.PI / 2 + ((i - 2) * Math.PI) / 8;
          const bx = ps.x + Math.cos(a) * 18;
          const by = cy - 58 + Math.sin(a) * 6;
          ctx.beginPath();
          ctx.moveTo(bx, by + 10);
          ctx.lineTo(bx + Math.cos(a) * 14, by + Math.sin(a) * 14 - 8);
      ...
      if (tier >= 4) {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(ps.x, cy - 62, 22, Math.PI * 0.15, Math.PI * 0.85, true);
        ctx.stroke();
        for (let i = 0; i < 8; i++) {
          const a = -state.time * 1.8 + (i * TAU) / 8;
          const mx = ps.x + Math.cos(a) * 38;
          const my = cy - 6 + Math.sin(a) * 14 + Math.cos(state.time * 2 + i) * 8;
          const mg = ctx.createRadialGradient(mx, my, 1, mx, my, 7);
          mg.addColorStop(0, "#fff6d0");
          mg.addColorStop(0.5, "#ff9020");
      ...
      for (let i = 0; i < gemN; i++) {
        const a = state.time * 1.35 + (i * TAU) / gemN;
        const gx = ps.x + Math.cos(a) * gemR;
        const gy = cy + Math.sin(a * 0.9) * 12 + Math.sin(state.time * 2 + i) * 6;
        const g = ctx.createRadialGradient(gx, gy, 1, gx, gy, tier >= 3 ? 9 : 8);
        g.addColorStop(0, "#fff6d0");
        g.addColorStop(0.45, tier >= 4 ? "#ff9020" : "#ffb040");
        g.addColorStop(1, "rgba(255,140,40,0)");
        ctx.fillStyle = g;
        ctx.beginPath();
        ctx.arc(gx, gy, tier >= 3 ? 9 : 8, 0, TAU);
        ctx.fill();
      }
      ctx.restore();
    }
