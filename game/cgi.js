...
(function () {
  ...
  function buildForestProps() {
    ...
    for (let i = 0; i < 160; i++) {
      ...
      if (!placeClear(wx, wy, 200)) continue;
  ...
  function init(container) {
    ...
    // Keep fill extremely low so the lantern is the primary reveal
    hemi = new THREE.HemisphereLight(0x3a4a40, 0x080604, 0.12);
    scene.add(hemi);
    ambient = new THREE.AmbientLight(0x101410, 0.05);
    scene.add(ambient);

    rimLight = new THREE.DirectionalLight(0x2a3830, 0.08);
    ...
    // Warm lantern as the dominant light — short range, soft decay
    lanternLight = new THREE.PointLight(0xffb040, 78, 26, 1.65);
    lanternLight.castShadow = true;
    lanternLight.shadow.mapSize.set(1024, 1024);
    lanternLight.shadow.bias = -0.0008;
    lanternLight.shadow.camera.near = 0.2;
    lanternLight.shadow.camera.far = 28;
    scene.add(lanternLight);

    flameLight = new THREE.PointLight(0xffe08a, 18, 8, 2.1);
  ...
  function sync(state, mode) {
    ...
    lanternLight.intensity = 72 * flick * lightMul;
    lanternLight.distance = 22 + evo.stage * 1.5;
    flameLight.position.set(pp.x, flameH + bob, pp.z);
    flameLight.intensity = 16 * flick * lightMul;
